...
Note that the label_data() function applies labelled::set_variable_labels() — it designed for cases where (1) you are working in R with packages that support variable labels (e.g., haven, labelled --- see below) or (2) you plan to export your data to environments like SPSS or SAS that can read and display variable labels. If you are writing the data to a .csv file, you will not see a difference since label metadata are unsupported.
