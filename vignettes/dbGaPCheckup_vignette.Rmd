...
In this example, dbGaPCheckup informs us that while the variable names match between the data dictionary and the data, they are in the wrong order. Instead of fixing this issue manually, we can simply call the `reoder_dictionary` function as a "quick fix" and run the `name_report` function to confirm! 
